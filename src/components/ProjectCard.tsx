      ...
      <div className="chat-scroll flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto p-4">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <p className={`max-w-[85%] px-4 py-2.5 text-[13px] leading-relaxed shadow-sm ${m.role === "user"
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="flex items-center gap-1.5 rounded-2xl rounded-tl-sm border border-white/10 bg-white/[0.06] px-4 py-3 backdrop-blur-md">
...
export default function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <article className="surface-card h-full flex flex-col orange-glow-hover group overflow-hidden rounded-xl transition-all duration-200">
      ...
      {aiOpen && (
        <Portal>
          <div className="fixed inset-0 z-50 flex items-end justify-end bg-black/60 p-5 transition-opacity" onClick={() => setAiOpen(false)}>
            <div className="flex h-[560px] w-[480px] flex-col overflow-hidden rounded-2xl border border-[var(--line-border)] bg-[#0c0804]/90 shadow-[0_8px_32px_rgba(255,106,0,0.15)] ring-1 ring-white/5 backdrop-blur-xl" onClick={(e) => e.stopPropagation()}>
